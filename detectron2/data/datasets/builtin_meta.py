...
# added by nshimada on 2022/08/20
DLA_CLS_CATEGORIES = [
    {"supercategory": "plant", "id": 1, "name": "leaf", "color": [119, 11, 32]},
]
...
# added by nshimada on 2022/08/20
def _get_dla_cls_instances_meta():
    thing_ids = [k["id"] for k in DLA_CLS_CATEGORIES]
    thing_colors = [k["color"] for k in DLA_CLS_CATEGORIES]
    assert len(thing_ids) == 1, len(thing_ids)
    # Mapping from the incontiguous COCO category id to an id in [0, ]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in DLA_CLS_CATEGORIES]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
        "thing_colors": thing_colors,
...
def _get_builtin_metadata(dataset_name):
    ...
    elif dataset_name == "dla_cls":
        return _get_dla_cls_instances_meta()
